...
<div class="container mx-auto px-4 py-6">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($spsForApproval as $sp)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-500">{{ \Carbon\Carbon::parse($sp->tgl_mulai)->format('d M Y') }} - {{ \Carbon\Carbon::parse($sp->tgl_selesai)->format('d M Y') }}</td>
                    ...
                    @empty
                        <tr><td colspan="6" class="py-8 text-center text-gray-500">Tidak ada Surat Peringatan yang menunggu persetujuan Anda.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="my-8 overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 shadow-sm sm:px-6">
        {{-- ... (kode form download tidak berubah, sudah benar) ... --}}
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($spsHistory as $sp)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <span @class([ 'px-2 py-1 text-xs font-medium rounded-full', 'bg-green-100 text-green-800' => $sp->status_gm == 'Disetujui', 'bg-red-100 text-red-800' => $sp->status_gm == 'Ditolak', 'bg-yellow-100 text-yellow-800' => $sp->status_gm == 'Menunggu Persetujuan', 'bg-gray-100 text-gray-800' => $sp->status_gm == null || $sp->status_gm == 'Menunggu' ])>
                                    {{ $sp->status_gm ?? 'N/A' }}
                                </span>
                            ...
                            <td class="px-4 py-3">
                                <span @class([ 'px-2 py-1 text-xs font-medium rounded-full', 'bg-green-100 text-green-800' => $sp->status_gm == 'Disetujui', 'bg-red-100 text-red-800' => $sp->status_gm == 'Ditolak' || $sp->status_sdm == 'Ditolak SDM', 'bg-gray-100 text-gray-800' => true ])>
                                    @if($sp->status_gm == 'Disetujui')
                                        Selesai
                                    @elseif($sp->status_gm == 'Ditolak' || $sp->status_sdm == 'Ditolak SDM')
                                        Ditolak
                                    @else
                                        -
                                    @endif

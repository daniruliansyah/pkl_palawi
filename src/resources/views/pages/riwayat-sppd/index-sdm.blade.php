...
@section('title', 'Persetujuan Sppd SDM')
...
<div class="container mx-auto px-4 py-6">
    ...
    {{-- Tabel 1: Daftar Pengajuan SPPD (Menunggu Persetujuan) --}}
                            ...
                            <td class="px-4 py-3 text-center" x-data="{ openModal: false }">
                                ...
                                <div x-show="openModal" @click.away="openModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" style="display: none;">
                                    <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                                        ...
                                        <form action="{{ route('approvals.update', $sppd->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
    ...
    <div class="mb-8 overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 shadow-sm sm:px-6">
        <h3 class="text-lg font-semibold text-gray-800">Download Laporan Arsip Surat Perjalanan Dinas (SPPD)</h3>
        <p class="text-sm text-gray-500 mb-4">Pilih periode untuk mengunduh semua surat SPPD yang telah disetujui dalam format ZIP.</p>
        {{-- PASTIKAN ROUTE INI BENAR UNTUK SPPD --}}
            ...
            <div>
                ...
                <select name="bulan" id="bulan" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    @foreach(range(1, 12) as $month)
                        {{-- Menggunakan Carbon untuk menampilkan nama bulan --}}
    ...
    {{-- Tabel 2: Riwayat Pengajuan SPPD --}}
            ...
            <table class="min-w-full text-sm text-left">
                    <thead>
                        <tr class="border-y border-gray-100 bg-gray-50 text-gray-600">
                            <th class="py-3 px-4 font-medium">Pemohon</th>
                            <th class="py-3 px-4 font-medium">Jenis Izin</th>
                            <th class="py-3 px-4 font-medium">Tanggal Berangkat</th> {{-- Diubah dari 'Tanggal Cuti' --}}
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                            <tr><td colspan="5" class="py-8 text-center text-gray-500">Belum ada riwayat pengajuan SPPD yang selesai diproses.</td></tr>
                        @endforelse
                    </tbody>
            </table>
        </div>

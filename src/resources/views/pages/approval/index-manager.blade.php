...
<div class="mb-8 overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 shadow-sm sm:px-6">
    <div class="mb-4">
        <h3 class="text-lg font-semibold text-gray-800">Daftar Pengajuan Cuti (Persetujuan Final)</h3>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center" x-data="{ openModal: false }">
                            <div class="flex items-center justify-center space-x-2">
                                {{-- Menggunakan route 'approvals.update' (dari ApprovalController) --}}
                                <form action="{{ route('approvals.cuti.update', $cuti->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menyetujui pengajuan ini?');">
                                    @csrf
                                    @method('PUT')
                            ...
                            <div x-show="openModal" @click.away="openModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" style="display: none;">
                                <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                                    ...
                                    <form action="{{ route('approvals.cuti.update', $cuti->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($cutisHistory as $cuti)
                    <tr class="hover:bg-gray-50">
                        ...
                        {{-- KOLOM KEPUTUSAN ANDA (MANAGER) --}}
                        <td class="px-4 py-3">
                            <span @class([
                                'px-2 py-1 text-xs font-medium rounded-full',
                                'bg-green-100 text-green-800' => $cuti->status_manager == 'Disetujui',
                                'bg-red-100 text-red-800' => $cuti->status_manager == 'Ditolak',
                            ])>{{ $cuti->status_manager }}</span>
                            @if($cuti->tgl_persetujuan_manager)
                            <span class="block text-xs text-gray-400 mt-1">{{ \Carbon\Carbon::parse($cuti->tgl_persetujuan_manager)->format('d M Y') }}</span>
                            @endif
                        </td>

                        {{-- KOLOM STATUS FINAL (LOGIKA BARU) --}}
                        <td class="px-4 py-3">
                            @php
                                $statusFinal = 'Menunggu';
                                $color = 'text-yellow-600';

                                // Cek Alur 3 (Pemohon SDM)
                                if ($cuti->user && $cuti->user->isSdm()) {
                                    if ($cuti->status_gm == 'Disetujui') {
                                        $statusFinal = 'Disetujui Penuh';
                                        $color = 'text-green-600';
                                    } elseif ($cuti->status_gm == 'Ditolak') {
                                        $statusFinal = 'Ditolak GM';
                                        $color = 'text-red-600';
                                ...
                                elseif ($cuti->user && $cuti->user->isGm()) {
                                    if ($cuti->status_manager == 'Disetujui') {
                                        $statusFinal = 'Disetujui Penuh';
                                        $color = 'text-green-600';
                                    }
                                }
                                
                                // Jika Manager menolak (berlaku untuk semua alur yg dia handle)
                                if ($cuti->status_manager == 'Ditolak') {
                                    $statusFinal = 'Ditolak Oleh Anda';
                                    $color = 'text-red-600';
                                }
                            @endphp
                            <span class="font-semibold {{ $color }}">{{ $statusFinal }}</span>
                        </td>

                        <td class="px-4 py-3 text-center">
                            @if( ($cuti->status_gm == 'Disetujui' || $cuti->status_manager == 'Disetujui') && $cuti->file_surat)
                ...
                @empty
                    <tr><td colspan="6" class="py-8 text-center text-gray-500">Anda belum memiliki riwayat tindakan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
